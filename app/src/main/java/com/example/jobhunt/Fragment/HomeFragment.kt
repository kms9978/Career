...
// 리사이클러뷰 초기화
        ...
        recyclerView2.layoutManager = GridLayoutManager(context, 2, GridLayoutManager.VERTICAL, false)
        searchEditText = view.findViewById(R.id.search_company)

        // 최근 채용 정보 데이터를 읽어온다
        recentRecruitList = readRecentRecruitData()

        //포지션이 신입만 해당되는 데이터 읽기
        newcomRecruitList = readnewcomRecruitData()

        // 어댑터를 리사이클러뷰에 설정한다
        recyclerView.adapter = RecentRecruitAdapter(recentRecruitList)
        recyclerView2.adapter = NewComeRecruitAdapter(newcomRecruitList)
